...
Se l'algoritmo viene richiamato con frequenza casuale, possono essere presenti molti cicli nel grafo delle risorse, e quindi non si può stabilire quale dei thread coinvolti nel ciclo abbia "causato" il deadlock.
...
Per ripristinare un deadlock bisogna terminare in _abort_ tutti i thread in deadlock uno alla volta, fino ad eliminare il ciclo di deadlock. L'ordine con cui terminare i thread, ovvero l'==ORDINE DI PRELAZIONE==, viene determinato in base ad alcuni fattori fondamentali:
- priorità del thread
- tempo di computazione trascorso e tempo rimanente al completamento del thread
- quantità e tipo di risorse impiegate (se sono non oppure facilmente [[Risorse#PREEMPTION PRELAZIONE|prelazionabili]])
